...
1. `app/portal-cliente/lotes/[id]/page.tsx` — **HECHO (08/09)**: cada fila de cuota lleva ahora ícono de estado (check verde = pagada, reloj azul + acento azul + botón "Pagar cuota" con ícono de tarjeta = actual, punto gris = futura) y la conversión a ARS por fila en el saldo. Encabezado con etiqueta de loteo + "Manzana X · Lote Y" (cae al identificador si el lote no tiene esos datos). "Mis pagos" pasó a tarjetas (ícono, monto, `motivo · medio · fecha`, badge Confirmado/Pendiente, link a comprobante/recibo). Se mantuvo la `<table>` de cuotas (semántica + los ~7 specs que navegan por `main table > tbody tr > "Pagar cuota"`) y todos los textos que testean los e2e ("Refinanció", "Falta subir comprobante", "Ver comprobante", "Ver recibo"). No se tocó `cuotasConDatos`, `pagosConLink`, `puedeEliminar` ni el filtro por `ciclo_actual`.
